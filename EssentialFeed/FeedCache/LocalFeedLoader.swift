...
extension LocalFeedLoader: FeedLoader {
    ...
    public func load(completion: @escaping (LoadResult) -> ()) {
        store.retrieve { [weak self] result in
            ...
            case let .success(.found(feed, timestamp)) where FeedCachePolocy.validate(timestamp, against: self.currentDate()):
...
extension LocalFeedLoader {
    ...
    public func validateCache() {
        store.retrieve { [weak self] result in
            ...
            case let .success(.found(_ , timestamp)) where !FeedCachePolocy.validate(timestamp, against: self.currentDate()):
                self.store.deleteCachedFeed { _ in}
